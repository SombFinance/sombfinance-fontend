...
export class TombFinance {
  ...
  constructor(cfg: Configuration) {
    ...
    this.SOMB = new ERC20(deployments.tomb.address, provider, 'SOMB');
    ...
    // Uniswap V2 Pair
    this.TOMBWFTM_LP = new Contract(externalTokens['SOMB-FTM-LP'][0], IUniswapV2PairABI, provider);
  ...
  unlockWallet(provider: any, account: string) {
    ...
    const tokens = [this.SOMB, this.SSHARE, this.SBOND, ...Object.values(this.externalTokens)];
  ...
  async getSombStat(): Promise<TokenStat> {
    const {TombFtmRewardPool, TombFtmLpTombRewardPool, TombFtmLpTombRewardPoolOld} = this.contracts;
    const supply = await this.SOMB.totalSupply();
    const tombRewardPoolSupply = await this.SOMB.balanceOf(TombFtmRewardPool.address);
    const tombRewardPoolSupply2 = await this.SOMB.balanceOf(TombFtmLpTombRewardPool.address);
    const tombRewardPoolSupplyOld = await this.SOMB.balanceOf(TombFtmLpTombRewardPoolOld.address);
    const tombCirculatingSupply = supply
      .sub(tombRewardPoolSupply)
      .sub(tombRewardPoolSupply2)
      .sub(tombRewardPoolSupplyOld);
    const priceInFTM = await this.getTokenPriceFromPancakeswap(this.SOMB);
    ...
    return {
      ...
      totalSupply: getDisplayBalance(supply, this.SOMB.decimal, 0),
      circulatingSupply: getDisplayBalance(tombCirculatingSupply, this.SOMB.decimal, 0),
    };
  }
  ...
  async getLPStat(name: string): Promise<LPStat> {
    ...
    const token0 = name.startsWith('SOMB') ? this.SOMB : this.SSHARE;
    const isSomb = name.startsWith('SOMB');
    ...
    const lpTokenPrice = await this.getLPTokenPrice(lpToken, token0, isSomb);
  ...
  async getBondStat(): Promise<TokenStat> {
    ...
    const tombStat = await this.getSombStat();
    const bondTombRatioBN = await Treasury.gesBondPremiumRate();
  ...
  async getShareStat(): Promise<TokenStat> {
    ...
    const tombRewardPoolSupply = await this.SSHARE.balanceOf(TombFtmLPTShareRewardPool.address);
  ...
  async getTombStatInEstimatedTWAP(): Promise<TokenStat> {
    ...
    const expectedPrice = await SeigniorageOracle.twap(this.SOMB.address, ethers.utils.parseEther('1'));
  ...
  async getTombPriceInLastTWAP(): Promise<BigNumber> {
    const {Treasury} = this.contracts;
    return Treasury.getSombUpdatedPrice();
  }

  async getBondsPurchasable(): Promise<BigNumber> {
    const {Treasury} = this.contracts;
    return Treasury.getBurnableSombLeft();
  }
  ...
  async getPoolAPRs(bank: Bank): Promise<PoolStats> {
    ...
    const stat = bank.earnTokenName === 'SOMB' ? await this.getSombStat() : await this.getShareStat();
  ...
  async getTokenPerSecond(
    ...
    if (depositTokenName.startsWith('SOMB')) {
  ...
  async getDepositTokenPriceInDollars(tokenName: string, token: ERC20) {
    ...
    if (tokenName === 'WFTM') {
      ...
      if (tokenName === 'SOMB-FTM-LP') {
        tokenPrice = await this.getLPTokenPrice(token, this.SOMB, true);
  ...
  async getBondOraclePriceInLastTWAP(): Promise<BigNumber> {
    const {Treasury} = this.contracts;
    return Treasury.gesBondPremiumRate();
  }
  ...
  async getTotalValueLocked(): Promise<Number> {
    ...
    const masonryTVL = Number(getDisplayBalance(masonrytShareBalanceOf, this.SSHARE.decimal)) * Number(SSHAREPrice);
   ...
   * @param isSomb sanity check for usage of tomb token or tShare
   * @returns price of the LP token
   */
  async getLPTokenPrice(lpToken: ERC20, token: ERC20, isSomb: boolean): Promise<string> {
    const totalSupply = getFullDisplayBalance(await lpToken.totalSupply(), lpToken.decimal);
    //Get amount of tokenA
    const tokenSupply = getFullDisplayBalance(await token.balanceOf(lpToken.address), token.decimal);
    const stat = isSomb === true ? await this.getSombStat() : await this.getShareStat();
  ...
  async earnedFromBank(
    ...
    try {
      if (earnTokenName === 'SOMB') {
        return await pool.pendingSOMB(poolId, account);
      } else {
  ...
  async getMasonryAPR() {
    ...
    const SSHAREPrice = (await this.getShareStat()).priceInDollars;
    const TOMBPrice = (await this.getSombStat()).priceInDollars;
  ...
  async listenForRegulationsEvents(): Promise<any> {
    ...
    const boughtBondsFilter = Treasury.filters.BoughsBonds();
  ...
  async estimateZapIn(tokenName: string, lpName: string, amount: string): Promise<number[]> {
    ...
    if (tokenName === FTM_TICKER) {
      ...
      const token = tokenName === TOMB_TICKER ? this.SOMB : this.SSHARE;
